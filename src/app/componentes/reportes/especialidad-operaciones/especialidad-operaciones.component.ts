...
@Component({
  selector: 'app-especialidad-operaciones',
  templateUrl: './especialidad-operaciones.component.html',
  styleUrls: ['./especialidad-operaciones.component.css']
})
export class EspecialidadOperacionesComponent implements OnInit {
...
chartArmado: Chart;
  ...
  armarChart(resp){
    resp.forEach( dato=>{
      ...
      if(this.Contiene(this.chartEsp,dato.especialidad)==-1){
        this.chartEsp.push(new chart(dato.especialidad,[1]));
      }else{
        this.chartEsp[this.Contiene(this.chartEsp,dato.especialidad)].data[0]++;
      }
    })
  ...
  Descargar(){
    this.fileService.exportarExcel(this.configurar(this.chartEsp),'especialidadesOperaciones');
  ...
  configurar(lista){
   ...
   ExportarPDF(){
    var index=0;
    this.fileService.EscribirPDF("especialidad-operaciones",'especialidad-operaciones');
  }
